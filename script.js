...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (carousel) {
       ...
       nextButton.addEventListener('click', function() {
        ...
        if (slideIndex >= totalSlides) {
            slideIndex = 0; // Reset index if it exceeds the total number of slides
        }
        moveSlides();
        moveSlides2();
    });
    ...
    prevButton.addEventListener('click', function() {
        slideIndex--;
        if (slideIndex < 0) {
            slideIndex = totalSlides - 1; // Set index to the last slide if it goes below 0
        }
        moveSlides();
        moveSlides2();
    });
...
function toggleMenu() {
    ...
    if (menu && menu.classList.contains('menu-open')) {
        ...
        if (content) {
            content.style.transition = 'transform 0.5s ease-in-out'; // Add transition
            content.style.transform = 'translateY(0)'; // Reset translateY
    ...
    if (menu && !menu.classList.contains('menu-open')) {
        ...
        setTimeout(function() {
            if (backHomeLink) {
                backHomeLink.style.transition = ''; // Remove transition
            }
            if (coursePage) {
                coursePage.style.transition = ''; // Remove transition
            }
            if (recommendedBooks) {
                recommendedBooks.style.transition = ''; // Remove transition
            }
